...
export const ControlPanel: React.FC<ControlPanelProps> = ({ state, onToggle, searchQuery = '' }) => {
  ...
  const ActuatorButton = ({ 
    ...
    // Modern flat colors for active state
    ...
    return (
      ...
      >
        ...
        <span className="font-bold text-sm xl:text-base tracking-wide text-center whitespace-nowrap">{label}</span>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-3xl p-5 lg:p-6 xl:p-8 border border-gray-100 dark:border-slate-700 shadow-sm h-full min-h-[500px] flex flex-col w-full overflow-hidden">
      ...
      {/* Header & Toggle - Flexible layout to prevent overflow */}
      <div className="flex flex-wrap items-center justify-between mb-6 gap-4 w-full">
        ...
        {/* Segmented Control Toggle - Fixed width to prevent out-of-box */}
